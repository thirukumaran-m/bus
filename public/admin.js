...
function checkCollegeReached(busLat, busLng) {
  ...
  if (distance < 0.001) {
    document.getElementById("busStatus").innerText = "Bus Reached College";
  }
}
...
socket.on("busLocation", (data) => {
  ...
  document.getElementById("busStatus").innerText = data.busId + " On the Way";
});
setInterval(() => {
  const currentTime = Date.now();

  if (currentTime - lastLocationTime > 10000) {
    document.getElementById("busStatus").innerText = "Bus Status: Stopped";
  }
}, 3000);
